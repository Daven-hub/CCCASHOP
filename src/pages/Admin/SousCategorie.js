...
function SousCategorie() {
  ...
  const handleCreateSousCategorie = (data) => {
    //
  };
    ...
    <div className='flex flex-col gap-6'>
      <div className='flex justify-between items-center'>
        ...
        <div className="flex gap-42 items-center">
          ...
          <button onClick={() => setOpenCreate(true)} className='py-2.5 px-6 flex items-center gap-1.5 text-[.85rem] rounded-[7px] bg-primary font-semibold text-white'><FaPlus stroke={.5} /> Nouvelle sous-catégorie</button>
        </div>
      </div>

      <div className="bg-white flex flex-col gap-6 border rounded-[6px] p-7">
        <div className='flex justify-between items-center gap-4'>
          <h1 className='text-[1.6rem] font-bold text-primary'>Liste des sous catégories</h1>
            ...
            <TableBody>
              {produits.length === 0 ? (
                ...
                produits.map((species, index) => (
                  <TableRow className="hover:bg-primary/5 cursor-pointer" key={index}>
                    ...
                    <TableCell className="hidden md:table-cell">Informatique</TableCell>
                    <TableCell className="hidden md:table-cell">Ici se sont des contenues de videos liant les domaines IT</TableCell>
      ...
      <CreateSousCategorie
        ...
        onCreate={handleCreateSousCategorie}
      />
    </div>
  )
}
