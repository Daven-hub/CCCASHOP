  ...
  return (
    <div className='flex flex-col gap-6'>
      <div className='flex justify-between items-center'>
        <h1 className='flex items-center text-primary md:text-[1.7rem] font-bold gap-2.5'><FiPackage /> Clients </h1>
        <div className="flex gap-42 items-center">
          ...
          <button className='py-2.5 px-6 flex items-center gap-1.5 text-[.85rem] rounded-[7px] bg-primary font-semibold text-white'><FaPlus stroke={.5} /> Nouveau Client</button>
        </div>
      </div>
      ...
      <div className="bg-white flex flex-col gap-6 border rounded-[6px] p-7">
        <div className='flex justify-between items-center gap-4'>
            <h1 className='text-[1.6rem] font-bold text-primary'>Liste des clients disponibles</h1>
              ...
              {produits.length === 0 ? (
                ...
                produits.map((species, index) => (
                  <TableRow className="hover:bg-primary/10 cursor-pointer" key={index}>
                    ...
                    <TableCell><img className='w-16 h-14 object-cover rounded-[6px]' src={species?.image} alt={species?.id} /></TableCell>
