...
import { StarIcon, TruckIcon, ChevronRightIcon, Plus, Minus, RefreshCcw, MoveRight, CreditCard, Loader2 } from 'lucide-react'
...
import { createFavoris, getAllFavoris, removeFavoris } from '../../../store/slices/favoris.slice.js'
...
const GridePriceWithVariation = ({ detailProduit,variation,productAttributes,selectedVariant,selected,handleSelect }) => {
  return (
    <>
      ...
      {selectedVariant ? (
        <div className="mt-4 p-3 border rounded-xl bg-gray-50">
          ...
          <p><strong>Quantité :</strong> {selectedVariant.qte}</p>
          <p><strong>Prix :</strong> {selectedVariant.pu} €</p>
        </div>
      ) : (
      ...
      <div className='flex flex-col gap-2'>
        ...
        <span className='text-sm text-gray-500'>
          En stock : {detailProduit?.qte} unités
        </span>
      </div>
    </>
  )
  ...
  return (
    <>
      ...
      <span className='text-sm font-semibold text-gray-600'>
        Disponibilité : <mark className='bg-transparent font-bold text-green-500'>{detailProduit?.qte}</mark> en stock
      </span>
    </>
  )
}
...
const DetailsProduct = ({ product }) => {
  ...
  const infoDetail=[
    ...
    {
      label:"Catégories",
      value:getNameCategAndSubcateg(detailProduit?.idCategorie,detailProduit?.idsubcateg)
    },
    {
      label:"Tags",
  ...
  // const sousCateg = subcategories?.find(
  //   x => x.idsubcateg === detailProduit?.idsubcateg
  // )?.nom

  // const attributeDisponible= useAttributsDisponibles(subcategories,attributes,attributeValues)

  // const attributsSousCategorie =useMemo(() => {
  //         return attributeDisponible[sousCateg] || [] ;
  //       }, [attributeDisponible,sousCateg]);

  // console.log("detailProduit",detailProduit)
  // console.log("sousCateg",sousCateg)
  // console.log("attributeDisponible",attributeDisponible)
  // console.log("attributsSousCategorie",attributsSousCategorie)
  const [productAttributes, setProductAttributes] = useState([]);
  const [selected, setSelected] = useState({});
  const [selectedVariant, setSelectedVariant] = useState(null);
  useEffect(() => {
    const variantsForProduct = produitVariants.filter(v => v.idProduit === detailProduit?.idproduits);
    ...
    const valueIds = [...new Set(relatedVariantValues.map(vv => vv.idValues))];
    ...
    const grouped = attributes
      .filter(attr => relatedAttributeValues.some(av => av.idAttribute === attr.idAttribute))
      .map(attr => ({
        ...attr,
        values: relatedAttributeValues.filter(av => av.idAttribute === attr.idAttribute)
      }));

    setProductAttributes(grouped);
  }, [detailProduit?.idproduits, attributes, attributeValues, variantValues, produitVariants]);

  const handleSelect = (idAttribute, idValue) => {
    const newSelection = { ...selected, [idAttribute]: idValue };
    setSelected(newSelection);

    // Vérifie si toutes les valeurs sont choisies
    const allSelectedValues = Object.values(newSelection);

    // Trouver le variant qui correspond exactement à ces valeurs
    const variant = produitVariants.find(variant => {
      const variantValueIds = variantValues
        .filter(vv => vv.idVariant === variant.idVariant)
        .map(vv => vv.idValues);

      // vérifier que toutes les valeurs choisies sont dans cette combinaison
      return (
        ...
        <div className='grid grid-cols-1 lg:grid-cols-2 gap-10'>
          ...
          <div className='flex flex-col gap-5'>
            ...
            {detailProduit?.hasVariation === 0 ? (
              ...
              <GridePriceWithVariation variation={getPrixVariantSock(detailProduit?.idproduits)} selected={selected} selectedVariant={selectedVariant} handleSelect={handleSelect} productAttributes={productAttributes} detailProduit={detailProduit} />
            )}
            ...
            <div className='flex items-center gap-3 pb-7 border-b mt-4'>
              <div className='flex items-center'>
                <Button
                  variant='ghost'
                  size='icon'
                  // onClick={() => handleQuantityChange(item, item.qte - 1)}
              ...
              <Button className='flex-1 text-white'><ShoppingCart02Icon strokeWidth={2} size={24}/> Ajouter au panier </Button>
              <Button variant='outline' className='flex-1 text-text-primary px-4'>
                <CreditCard size={13} /> Acheter maintenant
              </Button>
              <Button onClick={()=>handleAddRemoveFavoris(detailProduit)} className={`flex-1 text-white`}>
                {loadingFavoris ? (
                                            <Loader2 className='animate-spin h-5 w-5 text-white' />
                                          ) : (
                                            <FavouriteIcon className={`${isFavoris(detailProduit?.idproduits)?'text-white fill-white':''}`} strokeWidth={2} size={24}/>
                                          )}
                </Button>
              <Button className='flex-1 text-white'><RefreshCcw size={24}/></Button>
            </div>

            <div className='bg-gray-100 rounded-[5px] py-7 px-6 flex justify-between gap-4 items-center'>
              <div className="flex items-center gap-3">
                <img className='w-[60px] h-[60px] rounded-full flex-shrink-0 object-cover' src={BaseUrl+detailVendeur?.profile} alt={detailVendeur?.shopname} />
                <div className='flex flex-col gap-1.5'>
                  <span className='text-[.8rem] leading-[1] text-gray-500'>Vendeur :</span>
                  <h3 className='text-secondary font-semibold text-[1.12rem] leading-[1.2]'>{detailVendeur?.shopname}</h3>
                  <StarRating rating={detailVendeur?.rating} size={12} />
                </div>
              </div>
              <NavLink className="flex items-center text-sm font-semibold gap-3 py-2.5 px-7 rounded-full text-white bg-secondary" to={'/fournisseurs/'+detailVendeur?.id}>Visiter la boutique <MoveRight size={14}/> </NavLink>
            </div> 
