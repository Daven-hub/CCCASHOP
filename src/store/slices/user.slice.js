...
export const updateUser = createAsyncThunk (
  ...
  async (datas, thunkAPI) => {
    try {
      ...
      return await userService.updateUser (datas);
    } catch (error) {
      const message =
        (error.response &&
          error.response.data &&
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase (getUserById.rejected, (state, action) => {
        state.userStatus= "error";
        state.userError = action.payload || "Impossible de charger l'utilisateur";
      })
      ...
      .addCase (deleteuser.fulfilled, (state, action) => {
        state.userStatus = "success";
      state.users = state.users.filter((post) => post.id !== action.payload.id);
      })
